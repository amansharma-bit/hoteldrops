...
function TrackingContent() {
  ...
  const hotels = isDubai ? DUBAI_HOTELS : GENERIC_HOTELS.map(h => ({ ...h, name: `${h.name} in ${city}` }));
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap" as const, gap: 12 }}>
            <div>
              ...
              <div className="sora" style={{ fontSize: isMobile ? 20 : 24, fontWeight: 800, color: NAVY }}>While you&apos;re here — explore deals in {city}</div>
              <div style={{ fontSize: 13, color: "#64748b", marginTop: 4 }}>Pre-negotiated member rates for your same dates.</div>
            </div>
            <button onClick={() => router.push(`/search-hotels`)} style={{ background: B, color: "#fff", border: "none", padding: "12px 24px", borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" as const }}>
              See all {city} deals →
            </button>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: 16, marginBottom: 32 }}>
            {hotels.map((hotel, i) => (
              <div key={i} onClick={() => router.push("/search-hotels")} style={{ background: "#fff", border: "1.5px solid #e2e8f0", borderRadius: 14, overflow: "hidden", cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", transition: "transform 0.2s, box-shadow 0.2s" }}>
                <div style={{ height: 160, overflow: "hidden", position: "relative" as const }}>
                  ...
                  <div style={{ position: "absolute" as const, top: 12, left: 12, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", padding: "4px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, color: "#fff" }}>{hotel.badge}</div>
                </div>
                <div style={{ padding: "14px 16px" }}>
                  <div style={{ color: "#f59e0b", fontSize: 12, marginBottom: 4 }}>★★★★★</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: NAVY, marginBottom: 8, lineHeight: 1.4 }}>{hotel.name}</div>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={B} strokeWidth="2"><polyline points="20 6 9 17 4 12"/></svg>
                    <span style={{ fontSize: 12, color: B, fontWeight: 600 }}>Member exclusive rate</span>
                  </div>
                </div>
              </div>
            ))}
