...
function HotelCard({
  ...
  const pricePerNight = hotel.minRate
    ? formatPrice(
        String(parseFloat(hotel.minRate) / numNights),
        hotel.currency
      )
    : null;
  const totalPrice = hotel.minRate
          ...
          <div>
            ...
            {totalPrice && numNights > 1 && (
              <p className="text-xs text-gray-400">
                {totalPrice} total · {numNights} nights · {guests}{" "}
...
function FilterSidebar({
  ...
  starFilter: number[];
  onStarFilter: (v: number[]) => void;
  sortBy: string;
  onSortBy: (v: string) => void;
}) {
  const toggleStar = (s: number) => {
    onStarFilter(
      starFilter.includes(s) ? starFilter.filter((x) => x !== s) : [...starFilter, s]
    );
  };
  ...
  const filtered = hotels
    .filter((h) => {
      if (starFilter.length) {
        const s = starCount(h.categoryName);
