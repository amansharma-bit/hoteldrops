...
const CANCEL_OPTIONS = [
  ...
  { code: 'partial',        label: 'Partial refund' },
          ...
          <div style={{ position: 'relative', zIndex: 1, background: '#fff', borderRadius: isMobile ? '16px 16px 0 0' : 16, width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto', marginTop: isMobile ? 'auto' : 60, animation: 'slideUp 0.25s ease', boxShadow: '0 32px 80px rgba(0,0,0,0.3)', ...(isMobile ? { position: 'fixed' as const, bottom: 0, left: 0, right: 0, marginTop: 0, borderRadius: '20px 20px 0 0', maxHeight: '92vh' } : {}) }}>
              ...
              <div>
                <div style={{ fontFamily: "'Sora',sans-serif", fontSize: 17, fontWeight: 700, color: NAVY }}>
                  {uploadStep === 1 ? 'Upload your hotel booking confirmation'
                    ...
                    : blockInfo?.reason === 'network_error' ? 'Connection issue'
            ...
            <div style={{ padding: '20px 24px 32px' }}>
              ...
              {uploadStep === 'blocked' && blockInfo && (
                <div>
                  {blockInfo.reason === 'not_hotel' && (<div><div style={{ background: '#fefce8', border: '1.5px solid #fde68a', borderRadius: 14, padding: 24, marginBottom: 20 }}><div style={{ width: 44, height: 44, borderRadius: 12, background: '#fde68a', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#92400e" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path d="M14 2v6h6M16 13H8M16 17H8M10 9H8"/></svg></div><div style={{ fontSize: 16, fontWeight: 700, color: '#92400e', marginBottom: 8, fontFamily: "'Sora',sans-serif" }}>This does not look like a hotel booking.</div><p style={{ fontSize: 13, color: '#78350f', lineHeight: 1.7 }}>Please upload a hotel booking confirmation — not a flight ticket, train ticket, restaurant receipt, or other document.</p></div><div style={{ display: 'flex', gap: 10 }}><button onClick={() => { setUploadStep(1); setFile(null); setFileSource(null); setBlockInfo(null); }} style={{ flex: 1, background: B, color: '#fff', border: 'none', padding: '13px', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Upload a hotel voucher</button><button onClick={() => { setExtracted(emptyExtracted()); setBlockInfo(null); setUploadStep(2); }} style={{ flex: 1, background: '#fff', color: B, border: '1.5px solid #bfdbfe', padding: '13px', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Enter manually</button></div></div>)}
                  {blockInfo.reason === 'checkin_passed' && (<div><div style={{ background: '#f1f5f9', border: '1.5px solid #e2e8f0', borderRadius: 14, padding: 24, marginBottom: 20 }}><div style={{ width: 44, height: 44, borderRadius: 12, background: '#e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#475569" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></div><div style={{ fontSize: 16, fontWeight: 700, color: NAVY, marginBottom: 8, fontFamily: "'Sora',sans-serif" }}>Check-in date has already passed.</div><p style={{ fontSize: 13, color: '#64748b', lineHeight: 1.7 }}>Your stay has already started or ended. rebuq tracks prices before check-in. For future bookings, upload right after you book!</p></div><button onClick={() => { setUploadStep(1); setFile(null); setFileSource(null); setExtracted(null); setBlockInfo(null); }} style={{ width: '100%', background: B, color: '#fff', border: 'none', padding: '13px', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Track a future booking</button></div>)}
                  {blockInfo.reason === 'non_refundable' && (<div><div style={{ background: '#fef2f2', border: '1.5px solid #fecaca', borderRadius: 14, padding: 24, marginBottom: 20 }}><div style={{ width: 44, height: 44, borderRadius: 12, background: '#fecaca', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#991b1b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg></div><div style={{ fontSize: 16, fontWeight: 700, color: '#991b1b', marginBottom: 8, fontFamily: "'Sora',sans-serif" }}>Non-refundable booking</div><p style={{ fontSize: 13, color: '#7f1d1d', lineHeight: 1.7 }}>rebuq can't help with non-refundable bookings — even if the price drops, you can't cancel and rebook. Next time, book a flexible rate so you can take advantage of any drops.</p></div><button onClick={() => { setUploadStep(1); setFile(null); setFileSource(null); setExtracted(null); setBlockInfo(null); }} style={{ width: '100%', background: B, color: '#fff', border: 'none', padding: '13px', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Try another booking</button></div>)}
                  {(blockInfo.reason === 'parse_error' || blockInfo.reason === 'poor_quality') && (<div><div style={{ background: '#f1f5f9', border: '1.5px solid #e2e8f0', borderRadius: 14, padding: 24, marginBottom: 20 }}><div style={{ width: 44, height: 44, borderRadius: 12, background: '#e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#475569" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg></div><div style={{ fontSize: 16, fontWeight: 700, color: NAVY, marginBottom: 8, fontFamily: "'Sora',sans-serif" }}>Could not read your voucher</div><p style={{ fontSize: 13, color: '#64748b', lineHeight: 1.7 }}>{blockInfo.message || 'The image may be blurry or the text is not clear enough. Try a clearer photo or enter your details manually.'}</p></div><div style={{ display: 'flex', gap: 10 }}><button onClick={() => { setUploadStep(1); setFile(null); setFileSource(null); setBlockInfo(null); }} style={{ flex: 1, background: B, color: '#fff', border: 'none', padding: '13px', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Try again</button><button onClick={() => { setExtracted(emptyExtracted()); setBlockInfo(null); setUploadStep(2); }} style={{ flex: 1, background: '#fff', color: B, border: '1.5px solid #bfdbfe', padding: '13px', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Enter manually</button></div></div>)}
                  {blockInfo.reason === 'duplicate' && (<div><div style={{ background: '#f0fdf4', border: '1.5px solid #bbf7d0', borderRadius: 14, padding: 24, marginBottom: 20 }}><div style={{ width: 44, height: 44, borderRadius: 12, background: '#bbf7d0', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#166534" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"/></svg></div><div style={{ fontSize: 16, fontWeight: 700, color: '#166534', marginBottom: 8, fontFamily: "'Sora',sans-serif" }}>Already tracking this booking</div><p style={{ fontSize: 13, color: '#14532d', lineHeight: 1.7 }}>{blockInfo.message || 'We are already watching this booking for you.'}</p></div><button onClick={() => window.location.href = '/dashboard'} style={{ width: '100%', background: B, color: '#fff', border: 'none', padding: '13px', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>View my dashboard</button></div>)}
                  {blockInfo.reason === 'network_error' && (<div><div style={{ background: '#fff7ed', border: '1.5px solid #fed7aa', borderRadius: 14, padding: 24, marginBottom: 20 }}><div style={{ width: 44, height: 44, borderRadius: 12, background: '#fed7aa', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#c2410c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg></div><div style={{ fontSize: 16, fontWeight: 700, color: '#c2410c', marginBottom: 8, fontFamily: "'Sora',sans-serif" }}>Connection issue</div><p style={{ fontSize: 13, color: '#7c2d12', lineHeight: 1.7 }}>{blockInfo.message}</p></div><div style={{ display: 'flex', gap: 10 }}><button onClick={() => { setBlockInfo(null); if (file) doScan(); else setUploadStep(1); }} style={{ flex: 1, background: B, color: '#fff', border: 'none', padding: '13px', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Try again</button><button onClick={() => { setExtracted(emptyExtracted()); setBlockInfo(null); setUploadStep(2); }} style={{ flex: 1, background: '#fff', color: B, border: '1.5px solid #bfdbfe', padding: '13px', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Enter manually</button></div></div>)}
                </div>
              )}
              ...
              {uploadStep === 'success' && (
                <div style={{ textAlign: 'center' as const, padding: '20px 0' }}>
                  <div style={{ width: 56, height: 56, borderRadius: 16, background: '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={B} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><path d="M22 4L12 14.01l-3-3"/></svg>
                  </div>
