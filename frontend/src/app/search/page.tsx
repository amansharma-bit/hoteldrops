      ...
      {!isMobile&&<div style={{background:"#fff",borderBottom:"1px solid #e2e8f0",padding:"10px 32px",position:"sticky",top:60,zIndex:200}}>
        <div style={{background:"#fff",border:"1.5px solid #e2e8f0",borderRadius:14,display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1.3fr auto",alignItems:"stretch",height:64,overflow:"visible",position:"relative",boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
          ...
          <div ref={destRef} className="sfd" style={{padding:"0 20px",borderRight:"1px solid #e2e8f0",display:"flex",flexDirection:"column",justifyContent:"center",borderRadius:"12px 0 0 12px",position:"relative"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:4}}><input value={destInput} onChange={e=>handleDestInput(e.target.value)} onFocus={()=>{if(destInput.length>=1)setShowDestDrop(true);}} onBlur={()=>{setTimeout(()=>{if(!destInput.trim())setDestInput(destination);setShowDestDrop(false);},200);}} onKeyDown={e=>{if(e.key==="Enter"){setShowDestDrop(false);handleSearch();}if(e.key==="Escape"){setDestInput(destination);setShowDestDrop(false);}}} placeholder="City or destination" style={{border:"none",outline:"none",fontFamily:"inherit",fontSize:15,fontWeight:600,color:NAVY,background:"transparent",padding:0,flex:1}}/>{destInput&&<button onClick={()=>{setDestInput("");setDestSuggestions([]);setShowDestDrop(false);}} style={{background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:18,padding:"0 2px",flexShrink:0,lineHeight:1}}>×</button>}</div>
                    ...
                    <div>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12}}>
                        ...
                        <div style={{textAlign:"right",flexShrink:0}}>
                          {price>0?(<><div style={{background:"#dcfce7",color:"#16a34a",fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:100,marginBottom:4,display:"inline-block"}}>{hotel.otaPriceINR&&hotel.otaPriceINR>price?`Members save ${formatINR(hotel.otaPriceINR-price)}`:"Member rate"}</div>{hotel.otaPriceINR&&hotel.otaPriceINR>price&&<div style={{fontSize:12,color:"#64748b",textDecoration:"line-through"}}>{formatINR(hotel.otaPriceINR)}</div>}<div className="sora" style={{fontSize:24,fontWeight:800,color:NAVY}}>{formatINR(price)}</div><div style={{fontSize:11,color:"#64748b",marginTop:2}}>Taxes included · per night</div>{hotel.hasBreakfast&&<div style={{display:"inline-flex",alignItems:"center",gap:4,background:YELLOW,color:"#92400e",fontSize:11,fontWeight:700,padding:"3px 8px",borderRadius:5,marginTop:6}}><IconBreakfast/> Free Breakfast</div>}</>):<div style={{fontSize:13,color:"#64748b"}}>Price on request</div>}
                        </div>
                      </div>
                    </div>
