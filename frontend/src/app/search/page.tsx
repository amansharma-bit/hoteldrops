...
interface Hotel{code:string|number;name:string;stars:number|null;minRate:number;currency:string;imageUrl?:string;address?:string;city?:string;chain?:string;rating?:number|null;latitude?:number|null;longitude?:number|null;distanceKm?:number|null;amenities?:string[];isRefundable?:boolean|null;hasBreakfast?:boolean;lowestPriceINR?:number;displayPriceINR?:number;otaPriceINR?:number;displayOtaPriceINR?:number;memberSaving?:number;taxesIncluded?:boolean;}
...
function MapView({hotels,checkIn,checkOut,filterProps,onClose,onHotelClick,isMobile,loadComplete}:{hotels:Hotel[];checkIn:string;checkOut:string;filterProps:FiltersPanelProps;onClose:()=>void;onHotelClick:(h:Hotel)=>void;isMobile:boolean;loadComplete:boolean;}){
  ...
  const handleMapSearch=(q:string)=>{markersRef.current.forEach(({el,hotel})=>{el.style.display=!q||hotel.name.toLowerCase().includes(q.toLowerCase())?"block":"none";});if(q&&mapInstanceRef.current){const h=hotelsWithCoords.find(h=>h.name.toLowerCase().includes(q.toLowerCase()));if(h){mapInstanceRef.current.flyTo({center:[h.longitude!,h.latitude!],zoom:15,speed:1.5});setTimeout(()=>selectHotel(h),600);}}else{setSelectedHotel(null);}};
...
function SearchResults(){
  ...
  const NIGHTS=checkIn&&checkOut?Math.max(1,Math.round((new Date(checkOut).getTime()-new Date(checkIn).getTime())/86400000)):1;
  const priceINR=(h:Hotel)=>Math.round(parseFloat(String(h.displayPriceINR||h.lowestPriceINR||h.minRate||0))/NIGHTS);
  ...
  return(
    <div style={{fontFamily:"'Inter',sans-serif",background:"#f8fafc",color:"#1e293b",minHeight:"100vh"}}>
      ...
      {showMap&&<MapView hotels={filteredHotels.length>0?filteredHotels:hotels} checkIn={checkIn} checkOut={checkOut} filterProps={filterProps} onClose={()=>setShowMap(false)} onHotelClick={h=>{setShowMap(false);handleHotelClick(h);}} isMobile={isMobile} loadComplete={loadComplete}/>}
                ...
                <div key={String(hotel.code)} className="hcard-m" onClick={()=>handleHotelClick(hotel)}>
                  ...
                  <div style={{padding:"14px 16px 16px"}}>
                    ...
                    <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between"}}>
                      <div>{price>0?(<>{hotel.displayOtaPriceINR&&hotel.displayOtaPriceINR>price?<div style={{background:"#dcfce7",color:"#16a34a",fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:100,marginBottom:4,display:"inline-block"}}>Members save {formatINR(hotel.displayOtaPriceINR-price)}</div>:<div style={{background:"#dcfce7",color:"#16a34a",fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:100,marginBottom:4,display:"inline-block"}}>{discount}% off</div>}<div style={{fontSize:12,color:"#94a3b8",textDecoration:"line-through"}}>{formatINR(hotel.displayOtaPriceINR&&hotel.displayOtaPriceINR>price?hotel.displayOtaPriceINR:Math.round(price*(1+discount/100)))}</div><div className="sora" style={{fontSize:22,fontWeight:800,color:NAVY}}>{formatINR(price)}</div><div style={{fontSize:11,color:"#64748b"}}>Taxes included · per night</div></>):<div style={{fontSize:13,color:"#64748b"}}>Price on request</div>}</div>
