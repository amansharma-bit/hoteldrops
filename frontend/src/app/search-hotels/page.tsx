...
function CalendarDayCell({ day, ds, isDisabled, isStart, isEnd, isOnly, isInRange, isToday, onDayClick }: DayCellProps) {
  let bg = "transparent";
  ...
  return (
    <div
      onClick={() => { if (!isDisabled) onDayClick(ds); }}
      style={{
        height: 40,
        display: "flex",
        alignItems: "center",
        ...
        {Array.from({ length: days }).map((_, i) => {
          ...
          return (
            <CalendarDayCell 
              ...
              day={day}
              ds={ds}
              isDisabled={ds < todayStr}
              isStart={ds === checkIn && !!checkOut}
              isEnd={ds === checkOut}
              isOnly={ds === checkIn && !checkOut}
              isInRange={!!(checkIn && checkOut && ds > checkIn && ds < checkOut)}
              isToday={ds === todayStr}
        ...
        <div className="fs">
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "20px 20px 0" }}>
            {Array.from({ length: 12 }).map((_, i) => {
              ...
              return (
                <CalendarMonthCard 
                  key={`${dm.getFullYear()}-${dm.getMonth()}`}
                  year={dm.getFullYear()}
                  month={dm.getMonth()}
                  todayStr={todayStr}
                  checkIn={checkIn}
