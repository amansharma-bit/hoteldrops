...
const TRENDING = [
  { city: "Dubai, UAE", hotel: "Atlantis The Palm", price: "₹28,400/night", tag: "Trending", photo: "https://images.unsplash.com/photo-1582719508461-905c673771fd?w=600&q=80&auto=format&fit=crop" },
  { city: "Bangkok, Thailand", hotel: "Capella Bangkok", price: "₹18,200/night", tag: "Best value", photo: "https://images.unsplash.com/photo-1563492065599-3520f775eeed?w=600&q=80&auto=format&fit=crop" },
  { city: "Maldives", hotel: "Soneva Fushi", price: "₹1,24,000/night", tag: "Luxury", photo: "https://images.unsplash.com/photo-1573843981267-be1999ff37cd?w=600&q=80&auto=format&fit=crop" },
];
    ...
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", background: "#fff", color: "#0a0a0f", minHeight: "100vh" }}>
      ...
      <section style={{ background: "#1447b8", padding: "56px 40px 72px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
          ...
          <form onSubmit={handleSearch} style={{ background: "#fff", borderRadius: 14, display: "flex", boxShadow: "0 8px 40px rgba(0,0,0,0.15)", position: "relative" }}>
              ...
              {focused && (
                <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", border: "1px solid #e5e7eb", borderRadius: "0 0 12px 12px", boxShadow: "0 8px 24px rgba(0,0,0,0.1)", zIndex: 100, overflow: "hidden" }}>
                  ...
                  {DESTINATIONS.filter(d => !destination || d.city.toLowerCase().includes(destination.toLowerCase())).map((d, i) => (
                    <button key={i} onMouseDown={() => { setDestination(d.city); setFocused(false); }}
                      style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "10px 16px", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", textAlign: "left" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14 }}>
            {DESTINATIONS.map((d, i) => (
              <button key={i} onClick={() => setDestination(d.city)}
                ...
                <div style={{ position: "absolute", top: 10, left: 10, background: "rgba(255,255,255,0.92)", borderRadius: 20, padding: "3px 10px", fontSize: 16 }}>{d.flag}</div>
                <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(to top, rgba(0,0,0,0.75) 0%, rgba(0,0,0,0) 100%)", padding: "16px 14px 14px" }}>
                  <div style={{ fontFamily: "'Clash Display', sans-serif", fontSize: 18, fontWeight: 700, color: "#fff" }}>{d.city}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
            {TRENDING.map((t, i) => (
              <div key={i} style={{ border: "1.5px solid #f0f0f5", borderRadius: 16, overflow: "hidden", cursor: "pointer" }}
                onClick={() => setDestination(t.city.split(",")[0])}>
                <div style={{ height: 200, position: "relative", overflow: "hidden" }}>
                  <img src={t.photo} alt={t.hotel} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} loading="lazy" />
                  <span style={{ position: "absolute", top: 12, left: 12, fontSize: 10, fontWeight: 700, textTransform: "uppercase", color: "#1447b8", background: "rgba(255,255,255,0.95)", padding: "3px 10px", borderRadius: 20 }}>{t.tag}</span>
                </div>
      ...
      <section style={{ background: "#f7f9fc", borderTop: "1px solid #eaeef2", padding: "48px 40px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr auto", alignItems: "center", gap: 40 }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: "#6b7280", lineHeight: 1.7, maxWidth: 480 }}>Upload your existing booking voucher. We&apos;ll monitor the rate 24/7 and WhatsApp you the moment it drops.</p>
          </div>
