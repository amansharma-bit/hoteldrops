    ...
    <div style={{ background: CREAM, minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: "'Inter',-apple-system,sans-serif", color: INK, WebkitFontSmoothing: 'antialiased', overflowX: 'hidden' }}>
      ...
      <nav style={{ maxWidth: 1280, margin: '0 auto', width: '100%', padding: '22px clamp(20px, 4vw, 44px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, ...anim(0) }}>
          <span style={{ fontFamily: "'Archivo',sans-serif", fontSize: 'clamp(26px, 3vw, 34px)', fontWeight: 800, color: INK, letterSpacing: '-0.03em' }}>
      ...
      {/* HERO — fully centered, like the SaaSsy reference */}
      <header style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', maxWidth: 1100, margin: '0 auto', width: '100%', padding: 'clamp(20px, 3vh, 40px) clamp(20px, 4vw, 44px)', boxSizing: 'border-box' }}>
        {/* Badge */}
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 9, background: '#fff', border: `1px solid ${HAIR}`, borderRadius: 30, padding: '7px 15px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', ...anim(0.14) }}>
        ...
        {/* Headline — heavy Archivo, centered */}
        <h1 style={{ fontFamily: "'Archivo',sans-serif", fontSize: 'clamp(36px, 5.8vw, 72px)', fontWeight: 800, lineHeight: 1.05, letterSpacing: '-0.035em', margin: '26px auto 0', maxWidth: 960, ...anim(0.24, true) }}>
          Recover up to <span style={{ color: SAPPHIRE }}>51% more margin</span> on bookings you've already made.
        </h1>

        {/* Subline */}
        <p style={{ fontSize: 'clamp(16px, 1.9vw, 20px)', color: GREY, margin: '26px auto 0', maxWidth: 640, lineHeight: 1.5, ...anim(0.42) }}>
