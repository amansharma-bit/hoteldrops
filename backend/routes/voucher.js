...
function buildExtractionPrompt(rates) {
  ...
  '',
  '## DATE EXTRACTION — BE LENIENT:',
  'Dates on OTA apps appear in many formats: "Sat, 27 Jun 2026", "27 Jun 26", "Jun 27", "27-Jun-2026", "27/06/2026", "06/27/2026", "27 June", "Sat 27 Jun" etc.',
  'ALWAYS convert to YYYY-MM-DD format. If year is missing or unclear from a camera photo, infer it: if month is in the past use next year, otherwise use current year (2026).',
  'If check-in date is visible but checkout is not, and total_nights is visible, calculate checkout = check_in + total_nights.',
  'If dates are slightly blurry but readable, still extract them — do not set null just because the image is imperfect.',
  'Search bar dates (e.g. "Sat, 27 Jun 2026" in GRN/MMT header) are the booking dates — always extract these.',
  '',
  '## DOCUMENT TYPES:',
  ...
  '- MakeMyTrip hotel_detail_top page: ALWAYS extract the price shown on the right side of the page.',
  '  Price formats: "₹9,046 For 1 Night & 1 Room with tax" → total_price_paid = 9046 (per night, tax inclusive)',
  '  "₹X,XXX For 1 Night & 1 Room" → extract X,XXX as price_per_night_incl_tax AND total_price_paid',
  '  Even if only per-night price is shown, extract it as total_price_paid (user will confirm total)',
  '  Crossed-out price (strikethrough) = original price, ignore it. Use the LOWER bold price shown.',
  '  If "Super Package" or similar badge is shown with a price, extract that price.',
  ...
  '',
  '## SEARCH BAR EXTRACTION (CRITICAL):',
  'The search bar at the top of OTA pages contains key booking parameters — ALWAYS read these:',
  '- MMT search bar format: "CITY, AREA OR PROPERTY | CHECK-IN: [date] | CHECK-OUT: [date] | ROOMS & GUESTS: X Room, Y Adults"',
  '- GRN search bar: "Adult(s): X, Child(s): Y" or similar',
  '- Extract num_adults, num_rooms from "X Room, Y Adults" pattern in search bar',
  '- "1 Room, 2 Adults" → num_rooms=1, num_adults=2',
  '- "2 Room, 4 Adults" → num_rooms=2, num_adults=4',
  '',
  '## PRICE EXTRACTION FOR MMT SEARCH/DETAIL PAGES:',
  '- MMT shows "₹5,900 For 1 Night & 1 Room with tax" → price_per_night_incl_tax=5900, this is already tax-inclusive',
  '- MMT crossed-out price (e.g. ₹6,649 struck through) is the original price — use the lower non-struck price',
  '- "For X Nights & Y Rooms" → total = displayed_price (already total, not per night)',
  '- Always extract the FINAL price shown (after any discount), not the crossed-out original',
  '',
