  ...
  try {
    ...
    const hotels = hotelList.map(h => {
      ...
      return {
        ...
        city: dest.city,
        stars: h.starRating ? parseInt(h.starRating) : null,
        minRate: minRateUSD ? (minRateUSD * INR_RATE).toFixed(0) : null,
        maxRate: null,
        currency: 'INR',
  ...
  try {
    ...
    const roomList = (hotelData.roomTypes || []).map(rt => ({
      ...
      rates: (rt.rates || []).map(rate => ({
        ...
        net: rate.retailRate?.total?.[0]?.amount || 0,
